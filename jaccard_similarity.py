...
def jaccardSim(list1, list2):
    ...
    if len(set1) != len(set2):
        raise ValueError("Both list parameters have to be of the same length and contain repeated terms!")

    I = set1.intersection(set2)
    U = set1.union(set2)
    return len(I)/len(U)

def jaccardSim_avg(list1, list2):
    """
    :param list1: is a list of values. Both list1 and list 2 have to be of the same length!
    :param list2: is a list of values. Both list1 and list 2 have to be of the same length!
    ...
    for d in range(1, len(list1)+1):
        jSim_sum += jaccardSim(list1[:d], list2[:d])
